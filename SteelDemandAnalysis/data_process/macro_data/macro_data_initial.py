...
def BDI_data():
    ...
    # 返回值：dataFrame
    #       属性：  BDI
    data_path = path_config.BDI_data
    BDI_df = pd.read_excel(data_path)
    BDI_df.set_index('时间', inplace=True)
    return BDI_df[['BDI']].loc[param_config.begin_time:param_config.end_time]
...
def PMI_data():
    ...
    #       属性：  PMI（月）
    #               PMI:新订单（月）
    data_path = path_config.macro_year_on_year_data
    PMI_df = pd.read_excel(data_path)
    PMI_df.set_index('时间', inplace=True)
    return PMI_df[['PMI（月）', 'PMI:新订单（月）']].loc[param_config.begin_time:param_config.end_time]
...
def macro_year_on_year_data():
    ...
    return macro_year_on_year_df[
               ['社会消费品零售总额:当月同比（月）', 'M1:同比（月）',
                'M2:同比（月）', 'CPI:当月同比（月）',
                'PPI:全部工业品:当月同比（月）']].loc[param_config.begin_time:param_config.end_time]
...
def macro_cumulative_year_on_year_data():
    ...
    return macro_cumulative_year_on_year_df[
               ['社会消费品零售总额:累计同比（月）(整理）']].loc[param_config.begin_time:param_config.end_time]

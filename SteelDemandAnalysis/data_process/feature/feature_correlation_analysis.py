...
def initial_attr_data():
    ...
    estate_cumulative_year_on_year_data = estate_data_initial.estate_cumulative_year_on_year_data()
    ...
    BDI_data = macro_data_initial.BDI_data()
    PMI_data = macro_data_initial.PMI_data()
    macro_year_on_year_data = macro_data_initial.macro_year_on_year_data()
    macro_cumulative_year_on_year_data = macro_data_initial.macro_cumulative_year_on_year_data()

    manufacturing_cumulative_year_on_year_data = manufacturing_data_initial.manufacturing_cumulative_year_on_year_data()
...
def cal_period_record(basic_df, variable_df):
    # 计算领先指标属性集
    # 输入：
...
def selected_label(label_df):
    # 基于领先期筛选数据
    # 输入：
    #       label_df: 原始标签数据dataFrame
    # 返回：
    #       train_data_label_df：筛选后的标签数据dataFrame
    return label_df[MAX_PERIOD:]


if __name__ == '__main__':
    # 数据初始化
    label_df = initial_label_data()
    feature_df = initial_attr_data()

    # 基于领先期数据调整
    period_record = cal_period_record(label_df, feature_df)
    selected_attr_df = selected_attr(feature_df, period_record)
    selected_label_df = selected_label(label_df)
